...
-(void)deathAct{
    ...
    if(arc4random()%100>50){
        ...
        CCParticleSystemQuad *em1 = [CCParticleSystemQuad particleWithFile:@"explosion.plist"];
        [em1 setPosition:self.position];
        [em1 setPositionType:kCCPositionTypeFree];
        [em1 setAutoRemoveOnFinish:YES];
        [em1 resetSystem];
        [self.parent addChild:em1 z:1000];
        [self endNPCDeath];
    }else{
...
-(SpaceItem*)dropItem{
    ...
    }else if(change>=75 && change<80){//pow 2 5%
        itemType = SpaceItemPower2;
    }else if(change>=80 && change<85){//pow 3 5%
        itemType = SpaceItemPower3;
